...
int main(int argc, char *argv[])
{
    ...
    cv::Mat translateImage = cv::Mat::zeros(frameImage.size(), CV_8UC3);
    ...
    cv::moveWindow("translate", 200, 200);

    cv::VideoWriter rec("rec.mov", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 20, frameImage.size());
    ...
    while (true)
    {
        ...
        cv::HoughLines(edgeImage, lines, 1, CV_PI / 180, 110);
        ...
        cv::warpAffine(originalImage, translateImage, rotateMat, translateImage.size(), cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
        ...
        rec << translateImage; // ビデオライタに画像出力
    }
