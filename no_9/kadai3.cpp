//g++ kadai3.cpp -std=c++11 `pkg-config --cflags --libs opencv4`
#include <iostream>
...
int main(int argc, char* argv[]) {
    cv::VideoCapture capture("./card.mov");
    if (!capture.isOpened()) {
    ...
    // ビデオライタ生成
    // h264 でエンコード
    cv::VideoWriter rec("kadai3.mp4", cv::VideoWriter::fourcc('a', 'v', 'c', '1'), 20, frameImage.size());
