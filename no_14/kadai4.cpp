//g++ kadai4.cpp -std=c++11 -framework OpenGL -framework GLUT `pkg-config --cflags --libs opencv4` -Wno-deprecated
...
void display()
{
  ...
  for (size_t i = 1; i < mouse_points.size(); ++i) {
    cv::line(frameImage, mouse_points[i-1], mouse_points[i], cv::Scalar(255, 255, 0), 2);
  }
