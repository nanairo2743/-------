...
int main(int argc, const char* argv[]){
    ...
    cv::Mat sourceImg = cv::imread("lenna_g.jpg", cv::IMREAD_GRAYSCALE);
    if (sourceImg.empty()) {
    ...
    // 画像を保存
    cv::imwrite("original.jpg", sourceImg);
    ...
    cv::imwrite("high_spc.jpg", spcImg);

    // ハイパスフィルタには2値化処理を行う
    cv::threshold(resultImg, resultImg, 110, 255, cv::THRESH_BINARY);

    cv::imwrite("high_result.jpg", resultImg);
    ...
    cv::split(cxMatrix, imgMatrix);
    cv::normalize(imgMatrix[0], resultImg, 0, 255, cv::NORM_MINMAX, CV_8U);
    cv::imwrite("low_spc.jpg", spcImg);
    cv::imwrite("low_result.jpg", resultImg);
